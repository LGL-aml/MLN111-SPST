...
export const answer = action({
  ...
  handler: async (ctx, args) => {
    ...
    // Thực tế: embeddings và chat completion đang “sống” ở các API version khác nhau.
    // - Embedding model `gemini-embedding-001` hiện hỗ trợ tốt trên v1beta.
    // - Generate model `gemini-1.5-flash` hiện hỗ trợ tốt trên v1.
